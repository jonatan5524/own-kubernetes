...
run: 
	...
	docker run --name kubelet --network bridge-kube -e ETCD_ENDPOINT=etcd -d jonatan5524/own-kubernetes:kubelet

stop: 
	docker stop etcd kube-api kubelet

rm:
	docker rm etcd kube-api kubelet
	rm -rf ./bin/*